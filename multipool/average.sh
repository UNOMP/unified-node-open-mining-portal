#!/bin/bash
counter=0
#                redis-cli -h 172.16.1.17 del Pool_Stats:CurrentRoundNXT
#                redis-cli -h 172.16.1.17 del Pool_Stats:CurrentRoundBTC
now="`date +%s`"
...
done< <(redis-cli -h 172.16.1.17 zrangebyscore statHistory $tenminsago $now)
...
echo "SHA: $total_sha Scrypt: $total_scrypt X11: $total_x11 Scrypt-N: $total_scryptn Keccak: $total_keccak" >>/home/an/unomp/unified/multipool/alerts/hashrate.log
 
redis-cli -h 172.16.1.17 zadd Pool_Stats:AvgHRs:sha $now $total_sha":"$now
redis-cli -h 172.16.1.17 zadd Pool_Stats:AvgHRs:scrypt $now $total_scrypt":"$now
redis-cli -h 172.16.1.17 zadd Pool_Stats:AvgHRs:x11 $now $total_x11":"$now
redis-cli -h 172.16.1.17 zadd Pool_Stats:AvgHRs:scryptn $now $total_scryptn":"$now
redis-cli -h 172.16.1.17 zadd Pool_Stats:AvgHRs:keccak $now $total_keccak":"$now
